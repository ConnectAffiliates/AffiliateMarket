    ...
    <nav class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
          <div class="flex">
            ...
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
              ...
              <template v-if="hasRole('affiliate')">
                <Link
                  :href="route('affiliate.sales')"
                  class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out"
                  :class="{ 'border-emerald-500 text-gray-900': route().current('affiliate.sales') }"
                >
                  My Sales
                </Link>
              </template>
      ...
      <div :class="{ 'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown }" class="sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
          ...
          <!-- Mobile: Affiliate-specific navigation -->
          <template v-if="hasRole('affiliate')">
            <Link
              :href="route('affiliate.sales')"
              class="block w-full pl-3 pr-4 py-2 border-l-4 border-transparent text-left text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out"
              :class="{ 'border-emerald-500 text-emerald-700 bg-emerald-50 focus:text-emerald-800 focus:bg-emerald-50 focus:border-emerald-700': route().current('affiliate.sales') }"
            >
              My Sales
            </Link>
          </template>
